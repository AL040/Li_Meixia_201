...
largest <- max(spread)
index <- which.max(spread)
paste ("The largest win spread is", largest, "when the Bears scored", bears[index], "points and the Opponent socred", opponent[index], "points.")
```
...
```{r}
game_score <- rbind (game_score, matrix (c(37, 9, 17, 17), ncol=2))
game_score
```
...
```{r}
for (game_index in 1:nrow(game_score)){
  if (game_score[game_index, 1] > game_score[game_index, 2]){
   print ("Win")
  } else if (game_score[game_index, 1] < game_score[game_index, 2]){
   print ("Loss")
  }
}
```
...
```{r}
number <- 0
for (a_game in 1:nrow(game_score)){
  if (game_score[a_game, 3]==0){
    number <- number+1
  } else {
    break
  }
...
```{r}
number_of_factors <- function (an_int){
  num_factor <- 0
  for (a_num in 1:an_int){
    if (an_int %% a_num ==0){
...
```{r}
area_rec <- function (a_length=48 , a_width = 48){
  area_inch <- a_length*a_width
  area_feet <- area_inch/144
  return (c(area_inch, area_feet))
}
results <- area_rec (102, 60)
paste0("The area in square inches is ", results[1], ". ", "The area in square feet is ", results[2], ".")
```
...
```
```{r}
most_checker <- function (string_vec){
  char_vec <- nchar (string_vec)
  # find the highest number of letters
  highest_num <- max(char_vec)
  return (string_vec[char_vec==highest_num])
}
most_checker (fruits)
```
...
```{r}
check_in <- function(a_val, a_vector){
  if (a_val %in% a_vector){
    return (result = TRUE)
  } else {
    return (result = FALSE)
  }
}
check_in (value, check)
```
```{r}
